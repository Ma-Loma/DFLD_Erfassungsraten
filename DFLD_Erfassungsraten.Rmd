...
Die von Herrn Weise bereitgestellten Daten lese ich in R ein.
...
read_csv_file <- function(file) {
  read_delim(
    ...
    delim = ";",
    escape_double = FALSE,
    col_types = cols(StatNr = col_number(),Lfd = col_number(),`ts(anf)` = col_time(format = "%H:%M:%S"),`ts(end)` = col_time(format = "%H:%M:%S")),
    trim_ws = TRUE,
...
Stationsliste<-all_data$StatNam %>% unique()
...
all_data %>% 
  ggplot(aes(x=DEN,y=Messschwelle,fill=factor(Typ)))+
  geom_violin()+
  facet_wrap(vars(StatNam))
