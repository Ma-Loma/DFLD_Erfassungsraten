...
preaf_data<-zeitstempeln(preaf_data)
...
```{r Scatterplots1, fig.cap="Graphische Darstellung der Daten mit präferiertem Fangradius", fig.width=6.7,fig.height=4, cache=TRUE}
...
```{r Scatterplots1f, fig.cap="Graphische Darstellung der Daten nach Fangradius", fig.width=6.7,fig.height=5.5, cache=TRUE}
...
```{r Scatterplots2, fig.cap="Graphische Darstellung der Daten mit präferiertem Fangradius nach gewählter Dauerschall-Analysemethode", fig.width=6.7,fig.height=4, cache=TRUE}
...
```{r Scatterplots3, fig.cap="Graphische Darstellung der Daten mit präferiertem Fangradius nach Schwelle", fig.width=6.7,fig.height=4, cache=TRUE}
...
DENIntervalle <-
  function(tagestafel, Anfangsdatum, Endedatum) {
    tagestafel %>%
     ...
     select(dazu, ZeitLabel, Intervall) %>%
    #  pivot_wider(names_from = ZeitLabel, values_from = Intervall)
    #   select(-dazu)
  }

# #Bei Zeitumstellung ist Dauer länger oder kürzer
# DENIntervalle(
#     DEN,
#     dmy("29.03.2025"),
#     dmy("30.03.2025")) %>% select(-IntervallAnf,-IntervallEnde)


stat <-
  full_data %>%
  left_join (Erfassungsdauern) %>%
  group_by(StatNam, Fangradius) %>%
  summarise(
    n1 = sum(StatusNr == 0),
    n2 = n(),
...
left_join(full_data,Erfassungsdauern) %>% 
  ggplot(aes(x=Fangradius,y=Dauer))+
  geom_line()+
  facet_wrap(vars(StatNam))
...
statN1N2<-full_data %>%
  group_by(Fangradius, StatNam) %>%
  summarise(n1 = sum(StatusNr == 0),n2=n(),Erfassungsquote=100*n1/n2, mittlererLASmax = mean(LasMax))

statN1N2 %>% 
  ggplot(aes(x=Fangradius,y=Erfassungsquote))+
  geom_line()+
  facet_wrap(vars(StatNam))

statN1N2 %>% 
  pivot_longer(n1:n2,names_to = "Größe",values_to = "Wert") %>% 
  ggplot(aes(x=Fangradius,y=Wert,color=Größe))+
  geom_line()+
  facet_grid(cols=vars(StatNam))

statN1N2 %>% 
  pivot_longer(Erfassungsquote:mittlererLASmax,names_to = "Größe",values_to = "Wert") %>% 
  ggplot(aes(x=Fangradius,y=Wert,color=Größe))+
  geom_line()+
  facet_grid(cols=vars(StatNam))
